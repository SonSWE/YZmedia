...
namespace YZmediaService
{
    public class LoginMem
    {
        ...
        /// </summary>
        private static readonly Dictionary<long, YZ_User_Info> c_dicUser = new();
        /// <summary>
        ...
        public static void SetUser(YZ_User_Info user)
        {
            lock (c_lock)
            {
                var key = (long)user.User_Id;
        ...
        public static YZ_User_Info? GetUser(long userPid)
        {
            lock (c_lock)
            {
                if (c_dicUser.ContainsKey(userPid))
        ...
        public static YZ_User_Info? GetUserByUserId(string userId)
        {
            lock (c_lock)
            {
                if (c_dicUser.Count > 0)
                {
                    foreach (var item in c_dicUser.Values)
                    {
                        ...
                        if (string.Equals(item.User_Id, userId))
                        {
                            return item;
                        }
                    }
